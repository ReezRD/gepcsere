...
//OpusThirdOffset
app.get("/OpusThirdOffset", (req, res) => {
  let sql = `
  select * from opus
    order by title
    limit 40
...
//StudentFirstOffset
app.get("/StudentFirstOffset", (req, res) => {
  let sql = `
  select * from student
    order by name
    limit 70`;
...
//StudentSecondOffset
app.get("/StudentSecondOffset", (req, res) => {
  let sql = `
  select * from student
    order by name
    limit 70
...
//StudentThirdOffset
app.get("/StudentThirdOffset", (req, res) => {
  let sql = `
  select * from student
    order by name
    limit 70
...
//StudentFourthOffset
app.get("/StudentFourthOffset", (req, res) => {
  let sql = `
  select * from student
    order by name
    limit 58
...
//OpusSearch
app.get("/OpusSearch", (req, res) => {
  let sql = `
  select * from opus
  where ? or ? or ? == writer or title or year`;
...
app.delete("/DeleteLoaning/:id", (req, res) => {
  const id = req.params.id;
  let sql = `
    DELETE FROM loaning
    WHERE id = ?`;
...
app.get("/StudentSearch/:keres", (req, res) => {
  const keres = `%${req.params.keres}%`;
  let sql = `
    SELECT * FROM student
    WHERE name like ? or year like ?`;
  ...
  pool.getConnection(function (error, connection) {
    ...
    connection.query(sql, [keres, keres], async function (error, results, fields) {
      if (error) {
        const message = "Opus sql error";
        sendingGetError(res, message);
        return;
      ...
      if (results.length == 0) {
        const message = `Not found id: ${keres}`;
        sendingGetError(res, message);
        return;
      }
      sendingGetById(res, null, results[0], keres);
    });
    connection.release();
  });
});
...
app.get("/Loaning", (req, res) => {
  let sql = `SELECT id, specimentid, studentid, DATE_FORMAT(away, '%Y.%m.%d') away, DATE_FORMAT(back, '%Y.%m.%d') back FROM loaning`;
...
app.get("/Loaning/:id", (req, res) => {
  const id = req.params.id;
  let sql = `
    SELECT id, specimentid, studentid, DATE_FORMAT(away, '%Y.%m.%d') away, DATE_FORMAT(back, '%Y.%m.%d') back FROM loaning
    WHERE id = ?`;
...
app.post("/Loaning", (req, res) => {
  const newR = {
    id: sanitizeHtml(req.body.id),
    specimentid: sanitizeHtml(req.body.specimentid),
  ...
  let sql = `
  INSERT loaning
  (id, specimentid, studentid, away, back)
  VALUES
  (?, ?, ?, ?, ?)
    `;
  ...
  pool.getConnection(function (error, connection) {
    ...
    connection.query(
      sql,
      [newR.id, newR.specimentid, newR.studentid, newR.away, newR.back],
      function (error, result, fields) {
        sendingPost(res, error, result, newR);
      }
    );
...
app.delete("/Specimen/:id", (req, res) => {
  const id = req.params.id;
  let sql = `
...
app.get("/Specimen/:id", (req, res) => {
  const id = req.params.id;
  let sql = `
    SELECT sId, opusid, price,  DATE_FORMAT(acquisition, '%Y.%m.%d') acquisition FROM specimen
    WHERE sId = ?`;
...
app.post("/Specimen", (req, res) => {
  const newR = {
    opusid: req.body.opusid,
    price: req.body.price,
    acquisition: sanitizeHtml(req.body.acquisition),
  };
  ...
  pool.getConnection(function (error, connection) {
    ...
    connection.query(
      sql,
      [ newR.opusid, newR.price, newR.acquisition],
      function (error, result, fields) {
        sendingPost(res, error, result, newR);
      }
    );
...
app.put("/Specimen/:id", (req, res) => {
  ...
  pool.getConnection(function (error, connection) {
    ...
    connection.query(
      sql,
      [ newR.opusid, newR.price, newR.acquisition, id ],
      function (error, result, fields) {
        sendingPut(res, error, result, id, newR);
      }
    );
...
app.get("/EverythingBySpecimenId/:id", (req, res) => {
  const id = req.params.id;
  let sql = `
  select o.id, o.writer, o.title, o.year, s.sId, s.opusid, s.price, DATE_FORMAT(s.acquisition, '%Y.%m.%d') acquisition, l.id, l.specimentid, l.studentid, DATE_FORMAT(l.away, '%Y.%m.%d') away, DATE_FORMAT(l.back, '%Y.%m.%d') back, st.id, st.name, st.year from opus o
  inner join specimen s on s.opusid = o.id
...
app.get("/SpecimenByOpusId/:id", (req, res) => {
  const id = req.params.id;
  let sql = `
  select sId, opusid, price,  DATE_FORMAT(acquisition, '%Y.%m.%d') acquisition from specimen
  where opusid = ?
